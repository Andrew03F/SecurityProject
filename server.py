...
class ServerGUI:
    def __init__(self, root):
        ...
        self.connect_button = tk.Button(root, text="Connect", command=self.connect_to_client)
    ...
    def connect_to_client(self):
        if not self.connected:
            ...
            messagebox.showinfo("Info", "Already connected to client.")
    ...
    def connect_to_server_socket(self, password):
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
                ...
                self.textbox.insert(tk.END, "Server listening for connection...\n")
                conn, addr = server_socket.accept()
                self.textbox.insert(tk.END, f"Connected to client: {addr}\n")
                salt = os.urandom(16)
    ...
    def send_message(self):
        if self.client_conn:
            ...
            self.client_conn.sendall(encrypted_message_iv + encrypted_message)
